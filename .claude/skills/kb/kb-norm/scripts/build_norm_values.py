...
그 밖의 항은 특례다. 항을 구분하지 않고 값을 뭉치면 기본값과 특례값이 섞인다.
    ...
    .venv/bin/python3 .claude/skills/kb/kb-norm/scripts/build_norm_values.py
"""
import hashlib
import json
import os
...
LAW_ZONE_BASIS = "국토의 계획 및 이용에 관한 법률 제36조제1항"
DECREE_ZONE_BASIS = "국토의 계획 및 이용에 관한 법률 시행령 제30조제1항"


def zone_basis_contract():
    """용도지역 21종과 법적 근거 두 집합을 계약에서 읽고 집합 등식을 강제한다."""
    with open(CONTRACT, encoding="utf-8") as f:
        contract = json.load(f)
    zones_list = contract["용도지역"]
    groups = contract["용도지역_법적근거"]
    article_iris = contract["용도지역_근거조문IRI"]
    expected_keys = {LAW_ZONE_BASIS, DECREE_ZONE_BASIS}
    if set(groups) != expected_keys or set(article_iris) != expected_keys:
        raise RuntimeError(
            "용도지역 법적 근거는 법 제36조제1항과 시행령 제30조제1항 두 집합이어야 한다")
    law_list = groups[LAW_ZONE_BASIS]
    decree_list = groups[DECREE_ZONE_BASIS]
    law = set(law_list)
    decree = set(decree_list)
    zones = set(zones_list)
    if len(zones_list) != len(zones) or len(law_list) != len(law) \
            or len(decree_list) != len(decree):
        raise RuntimeError("용도지역 또는 법적 근거 집합에 중복 원소가 있다")
    if len(law) != 5 or len(decree) != 16 or not law.isdisjoint(decree) \
            or law | decree != zones:
        raise RuntimeError(
            "법 제36조제1항 5종과 시행령 제30조제1항 16종은 서로소이고, "
            "합집합이 용도지역 21종과 정확히 같아야 한다")
    definitions = sorted(
        (article_iris[basis], zone)
        for basis, basis_zones in groups.items()
        for zone in basis_zones)
    return sorted(zones), definitions


def require_zone_basis_articles():
    """용도지역 근거 조문 둘이 delegation.ttl의 ArticleWork인지 확인한다."""
    import rdflib
    _, definitions = zone_basis_contract()
    graph = rdflib.Graph()
    graph.parse(DELEGATION_TTL, format="turtle")
    lp = rdflib.Namespace("https://w3id.org/lp/ont#")
    articles = {rdflib.URIRef(article) for article, _ in definitions}
    missing = sorted(
        str(article) for article in articles
        if (article, rdflib.RDF.type, lp.ArticleWork) not in graph)
    if missing:
        raise RuntimeError(
            "용도지역 법적 근거 조문이 delegation.ttl의 lp:ArticleWork가 아니다: "
            + ", ".join(missing))
...
# 둘 다 P.* 를 그대로 쓴다 — 여기서 따로 다시 정의하면 파서 본체(같은 경계로
# 괄호 조건부 값을 잘라내는 EXCEPTION_MARK_RE)와 갈라질 위험이 있다
# (norm-followup 2). 정본은 하나다.
EXCEPTION_RE = P.EXCEPTION_MARK_RE
EXCEPTION_VALUE_RE = P.PERCENT_VALUE_RE
        ...
        for _, label, text, _ in C.articles(doc):
            for para_no, body in P.split_paragraphs(text):
                ...
                if EXCEPTION_RE.search(body):
                    ...
                    # EXCEPTION_VALUE_RE(=P.PERCENT_VALUE_RE)는 이제 frac/pct 두
                    # named group 이라 .findall() 이 튜플을 낸다 — .finditer() +
                    # P.percent_value() 로 정규화된 값만 뽑는다.
                    miss = [v for v in
                            (P.percent_value(m) for m in EXCEPTION_VALUE_RE.finditer(body))
                            if v not in got]
                    if miss:
                        quarantine.append({
                            "사유": "예외값_미파싱",
...
def build_ttl(rows):
    ...
    zones, zone_definitions = zone_basis_contract()
    unknown_zones = sorted({r["용도지역"] for r in rows} - set(zones))
    if unknown_zones:
        raise RuntimeError(f"계약 밖 용도지역 값이 있다: {unknown_zones}")
    L += ["##  용도지역", ""]
    for z in zones:
        L += [f"<{M.rel(M.zone(z))}> a lp:UseZone ;", f'    rdfs:label "{z}"@ko .', ""]

    L += ["##  용도지역 법적 근거", ""]
    for article, zone in zone_definitions:
        L += [f"<{M.rel(article)}> lp:defines <{M.rel(M.zone(zone))}> ."]
    L.append("")
