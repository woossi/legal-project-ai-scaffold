...
> **재설계 중 (2026-08-25).** `output/kb/` 산출물이 전부 삭제됐다. 아래 스크립트·계약은
> 남아 있고 위임 사슬·규범값 추출 절차 자체는 유효하나, 산출 경로와 그래프 구조는
> 새 3계층 체계에서 다시 정한다. 상태는 `.claude/rules/계획규범요소-틀.md`가 정본이다.
...
- `references/common-mistakes.md` — 규칙을 없앴을 때 실제로 유입된 것
...
.venv/bin/python3 -m pytest tools/tests/test_kb_norm_*.py -q
```
...
- **가지조문(제84조의2 등)을 본조로 읽지 않는다.** `basis["number"]` 만 보면
  ...
  꼭 같이 본다. 실측 20건/19관할. `is_branch_article` 판정이 값 유무와
  무관하게 먼저 걸리도록 짜여 있어 `100분의` 지원 이후에도 안전하다 —
  20건 그대로 `가지조문_제외`다(확인됨)
- **천단위 쉼표를 떼고 읽는다.** `1,000퍼센트` 를 `[0-9]{1,4}` 로 읽으면 `000` 이 잡혀
  값이 0 이 된다. 실측 31건/17개 조례
- **`100분의 N`은 `N퍼센트`와 같은 값이다.** 100분의 40 = 40퍼센트(분수 표기).
  `percent_value()` 가 두 표기를 정규화한다 — 정본은 `parse_ordinance.py` 하나이고
  `build_norm_values.py` 의 예외 문언 스캔도 이걸 재사용한다. 실측 12엔트리/4관할
  (군포시·오산시·인천광역시·평택시)
- **`1천500퍼센트`도 값이다(한글 자릿수).** 1000 이상 값(고밀도 상업지역 용적률)에서만
  관측된다. 관측된 자릿수(`1천`·`1천N00`)만 지원한다 — `만` 등 관측 안 된 표기를 미리
  만들지 않는다. 실측 16엔트리/7관할(가평군·구리시·동두천시·부천시·서울특별시·여주시·하남시)
- **용도지역명 오탈자는 매칭 후보로 흡수하고 원문은 보존한다.** 고양시 제61조의
  `자연환경보존지역`(정본은 `보전`). `_ZONE_VARIANTS` 로 매칭·정규화하고
  `lp:근거발췌` 에는 원문 그대로 남긴다. 계약에만 적으면 매칭이 안 걸려 값이 여전히
  안 나온다. 전 corpus 실측 1건
- **값과 먼 괄호를 조건부 값으로 읽지 않는다.** 괄호가 값+연산자 바로 뒤(공백만 사이)일
  때만 조건부 값이다. `ITEM_RE` 가 어떤 호를 못 읽으면 앞 호 구간이 부풀어 뒤 호의
  괄호값을 가로챈다 — 실측으로 거짓 명제 2건(수원시)이 그래프에 있었다
...
- **원문에 없는 비교연산자를 기본값으로 채우지 않는다.** `20퍼센트`(연산자 표기
  ...
  `None`을 낸다 — `lp:비교연산_미표기`로 드러내고 격리하지 않는다. 현재 정본
  실측(2026-08-13, `0cef35b`)은 총 1,086명제 중 명시 915/미표기 171이다. 이
  배타성은 pytest 불변식과
  `sh:xone` 양쪽이 검사한다 — 그래프가 저장소 밖(pytest 없는 소비처)에서 SHACL 로만
  검증될 때 위반이 새기 때문이다. 둘 다 둔다
