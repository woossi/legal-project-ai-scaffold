...
def build():
    ...
    observation = R.resolve(DSTRC, md_text(d))
    ...
    return dict(term=t, district=d, occ=occ, occ_idx=occ_idx,
                observation=observation, act=act, dec=dec, ordn=ordn,
                unresolved=[N.normalize(c) for c in t["cited_laws"]
                            if N.normalize(c)["status"] == "unresolved"])


def ttl(x):
    t, d, occ, obs = x["term"], x["district"], x["occ"], x["observation"]
    ...
    L = ["@prefix lp:  <https://w3id.org/lp/ont#> .",
         ...
         "##  District 시간값은 관측용이다. 적용일·ArticleVersion이 미확정이라 LawApplication은 발급하지 않는다.",
         ""]
    ...
    L += [f"<district/{DSTRC}> a lp:District ;",
          ...
          f'    lp:observationBasis "{obs["basis"]}" ;',
          f'    lp:observationPrecision "{obs["precision"]}" ;',
          '    lp:applicableVersionUnresolved "true"^^xsd:boolean ;',
...
def dot(x):
    ...
    L += ['  "app" [label="lp:LawApplication\\n미발급 — 적용일·판본 미확정",'
          ' shape=box, style="rounded,filled,dashed", fillcolor="#f3f3f3", color="#888888",'
          ' fontcolor="#666666", penwidth=1.6, fontsize=11];', ""]
    ...
    L += [f'  "act" -> "dec" [label="lp:delegates 위임", color="{B}", fontcolor="{B}", penwidth=2.0];',
          ...
          f'  "app" -> "dec" [label="판본 확인 후", color="#888888", fontcolor="#888888", style=dashed];',
          f'  "app" -> "dist" [label="발급 전", color="#888888", fontcolor="#888888", style=dashed];',
          f'  "app" -> "occ" [label="발급 전", color="#888888", fontcolor="#888888", style=dashed];',
    ...
    L += ["  subgraph cluster_note {",
          ...
          f'② 지구번호 연도는 관측값일 뿐이다 — 실제 적용일과 판본이 확인되기 전에는 LawApplication을 발급하지 않는다</font></td></tr>',
