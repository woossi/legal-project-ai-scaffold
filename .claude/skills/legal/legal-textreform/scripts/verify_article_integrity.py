...
DEFAULT_SOURCE_DIR = ROOT / "output/legal/시행지침"
DEFAULT_TABLE_LOSS = ROOT / "output/legal/table/_table_loss.json"
...
HWP5TXT_FALLBACK = ROOT / ".venv/bin/hwp5txt"
...
REGEN_COST = {
    ...
    "gate12": "legal-statute 검증 게이트 12 재검증",
...
def resolve_hwp5txt(project_root: Path = ROOT) -> Path | None:
    env = os.environ.get("HWP5TXT")
    if env:
        return Path(env)
    path_hit = shutil.which("hwp5txt")
    if path_hit:
        return Path(path_hit)
    fallback = project_root / ".venv/bin/hwp5txt"
...
def is_noise_witness_line(line: str, exclude_generated_comments: bool = True) -> bool:
    s = line.strip()
    if not s:
        return True
    if exclude_generated_comments and s.startswith("<!-- source:") and s.endswith("-->"):
        return True
...
def prose_witnesses(source_body: str, exclude_generated_comments: bool = True) -> list[str]:
    witnesses = []
    for line in source_body.splitlines():
        if is_noise_witness_line(line, exclude_generated_comments):
            continue
...
def witness_coverage(
    source_body: str,
    markdown_doc_norm: str,
    markdown_line_norms: Iterable[str] = (),
    exclude_generated_comments: bool = True,
) -> tuple[float, list[str], int]:
    witnesses = prose_witnesses(source_body, exclude_generated_comments)
    if not witnesses:
        return 1.0, [], 0
    missing = []
    for witness in witnesses:
        witness_norm = normalize_text(witness)
        if witness_norm not in markdown_doc_norm and not any(
            witness_norm in line_norm for line_norm in markdown_line_norms
        ):
...
def compare_articles(
    ...
    # 표 셀과 규범 산문이 한 줄로 눌린 Markdown도 있다. 표행은 문서 전체
    # 비교에서는 제외하되, 물리적 표행 안의 산문 witness는 별도로 대조한다.
    md_table_line_norms = [
        normalize_text(line) for line in md_text.splitlines() if is_table_like_line(line)
    ]
    ...
    for src in source_articles:
        ...
        table_context = article_has_table_context(src)
        table_line_norms = (
            md_table_line_norms
            if title_present and table_context
            else ()
        )
        coverage, missing_witnesses, witness_count = witness_coverage(
            src.body,
            md_doc_norm,
            table_line_norms,
            exclude_generated_comments=table_context,
        )
...
def integrity_status_text(summary: dict) -> str:
    pending = []
    if summary.get("table_loss_status") == "pending_input":
        pending.append("표 손실 입력")
    contract_violations = summary.get("table_loss_contract_violation_count", 0)
    if contract_violations:
        pending.append(f"표 손실 입력 계약 위반 {contract_violations}건")
    if summary.get("norm_values_status") == "pending_input":
        pending.append("norm_values 입력")
    unverified = summary.get("unverified_documents", 0)
    if unverified:
        pending.append(f"{unverified}개 미검증 문서")
    if pending:
        return f"PARTIAL — {', '.join(pending)}가 남아 있음"
    return "COMPLETE — 조문과 표 손실 입력 검증이 완료됨"
...
def write_markdown_report(path: Path, report: dict) -> None:
    ...
    lines = [
        ...
        f"- 판정 상태: {integrity_status_text(summary)}",
...
def main(argv: list[str] | None = None) -> int:
    ...
    hwp5txt = resolve_hwp5txt(ROOT)
    if hwp5txt is not None:
        os.environ.setdefault("HWP5TXT", str(hwp5txt))
